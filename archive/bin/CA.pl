...
sub create_certificate_request
{
...
sub create_directory_hierarchy
{
...
my $RET = 0;
switch($WHAT) {
  case '-crl'           { generate_crl() }
  case '-newca'         { create_directory_hierarchy() }
  case '-newcert'       { create_certificate() }
  case '-newreq'        { next }
  case '-newreq-nodes'  { create_certificate_request() }
  case '-pkcs12'        { generate_pkcs12() }
  case '-precert'       { create_precertificate() }
  case '-revoke'        { revoke() }
  case '-sign'          { sign_certificate() }
  case '-signCA'        { sign_CA() }
  case '-signcert'      { sign_cert() }
  case '-verify'        { verify() }
  case '-xsign'         { xsign_certificate() }
  else {
